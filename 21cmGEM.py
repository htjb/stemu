...
# Load the data
X_train = np.loadtxt('downloaded_data/train_data.txt')[:100]
X_test = np.loadtxt('downloaded_data/test_data.txt')[:100]
y_train = np.loadtxt('downloaded_data/train_labels.txt')[:100]
...
[plt.plot(z, emu.predict(X_test)[i], c='r', ls=':') for i in range(5)]
plt.xlabel('z')
